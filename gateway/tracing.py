"""OpenTelemetry tracing for the P-Ork Gateway.
...
The key entry point for unified P-Ork → Gateway traces is extract_remote_context():
P-Ork injects a W3C traceparent into the agent WebSocket request params, and
the Gateway uses it to make agent.run a child of P-Ork's gen_ai.gateway span.
"""
...
tracer = trace.get_tracer("pork-gateway")

_DEFAULT_SERVICE_NAME = "pork-gateway"
...
def extract_remote_context(params: dict) -> otel_context.Context:
    """Extract W3C trace context injected by P-Ork into the WebSocket request params.

    P-Ork calls propagate.inject(params) before sending the agent request, which
    ...
    a child of P-Ork's gen_ai.gateway span.
